...
object FPQA2PreBnd {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // read data
    val data = ss.sparkContext.textFile("data/nor_trade_bndno_b.txt")

    val train = ss.sparkContext.textFile("data/nor_trade_train_bndno_b.txt")

    val test = ss.sparkContext.textFile("data/nor_trade_test_bndno_b.txt")
  ...
  def predict(arrays: Array[String], trainLines: Array[String], testLines: Array[String], singleSortedFreqs: Array[String]): Unit = {
    ...
    // output table
    val writer = new BufferedWriter(new FileWriter("data/nor_pre_rate_bnd.csv"))
    writer.write("vip,correct rate")
    writer.newLine()
    for (i <- rates.indices) {
      writer.write(i + "," + (rates(i) * 100).formatted("%.2f"))
      writer.newLine()
    }
    writer.close()
  }
