...
object FPQA2PrePlu {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // read data
    val data = ss.sparkContext.textFile("data/nor_trade_pluno_b.txt")

    val train = ss.sparkContext.textFile("data/nor_trade_train_pluno_b.txt")

    val test = ss.sparkContext.textFile("data/nor_trade_test_pluno_b.txt")
  ...
  def predict(arrays: Array[String], trainLines: Array[String], testLines: Array[String], singleSortedFreqs: Array[String]): Unit = {
    ...
    // output table
    val writer = new BufferedWriter(new FileWriter("data/nor_pre_rate_plu.csv"))
